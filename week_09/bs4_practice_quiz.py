...
########################################
tags = soup.select('p span') # space is descendent
problem2 = len(tags)
print("problem2=",problem2)

########################################
tags = soup.select('p, .bold') # when both appear together, only include once (remove duplicate); comma is or
problem3 = len(tags)
...
# similar to soup.select('p a')


########################################
tags1 = soup.select('p')
tags2 = soup.select('.bold') # does not remove duplictes; counted again; because it is another dedicated variable
tags = tags1 + tags2
problem5 = len(tags)
...
########################################
# Extra:
tags = soup.select('*') # * means matching everything
tags_filtered = [ tag for tag in tags if 'Lorem ipsum' in tag.text ]  # Note capitalisations
problem_extra = len(tags_filtered) # Will duplicate
